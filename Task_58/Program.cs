...
void PrintThirdMatrix(int[,] arrayTree)
{
    for (int k = 0; k < arrayTree.GetLength(0); k++)
    {
        for (int e = 0; e < arrayTree.GetLength(1); e++)
        {
            if (e == 0) Console.Write("[");
            if (e < arrayTree.GetLength(1) - 1) Console.Write(arrayTree[k, e] + ",");
            else Console.Write(arrayTree[k, e] + "]");
        }
        Console.WriteLine();
    }
}
